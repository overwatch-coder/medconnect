...
type EditOfficialGeneralInfoProps = {
  open: boolean;
  setOpen: React.Dispatch<React.SetStateAction<boolean>>;
  healthOfficial: HealthOfficialType;
};

const EditOfficialGeneralInfo = ({
  open,
  setOpen,
  healthOfficial,
}: EditOfficialGeneralInfoProps) => {
  ...
  const {
    ...
    formState: { errors },
    handleSubmit,
  } = useForm<Partial<HealthOfficialType>>({
    resolver: zodResolver(healthOfficialSchema.partial()),
    defaultValues: healthOfficial,
    mode: "all",
  });

  const { mutateAsync, isPending: pending } = useMutation({
    mutationFn: async (data: Partial<HealthOfficialType>) => {
      console.log({ data });
      return data;
    },

    onSuccess: (data) => {
      toast.success("Health Official General Information updated successfully");
      setOpen(false);
      setOpenEditAdditionalInfo(true);
      reset();
    },
  });

  const handleFormSubmit: SubmitHandler<Partial<HealthOfficialType>> = async (
    data
  ) => {
    console.log({ data });
...
export default EditOfficialGeneralInfo;

// Submit form button
const EditOfficialGeneralInfoButton = ({
  pending,
  reset,
}: {
  pending: boolean;
