...
type EditPatientAdditionalInfoProps = {
  open: boolean;
  setOpen: React.Dispatch<React.SetStateAction<boolean>>;
};

const EditPatientAdditionalInfo = ({
  open,
  setOpen,
}: EditPatientAdditionalInfoProps) => {
  ...
  const {
    ...
    handleSubmit,
  } = useForm<Partial<PatientAdditionalInformationType>>({
    resolver: zodResolver(patientAdditionalInformationSchema.partial()),
    mode: "all",
  });

  const handleFormSubmit: SubmitHandler<
    Partial<PatientAdditionalInformationType>
  > = async (data) => {
    console.log({ data });
    setOpen(false);
            ...
            <div className="flex flex-col gap-5 w-full">
              ...
              >
                <div className="flex flex-col gap-5 px-3 pt-5 pb-10 bg-white h-full">
                  ...
                  {/* Submit form button */}
                  <EditPatientAdditionalInfoButton
                    pending={pending}
                    reset={reset}
                  />
                </div>
...
export default EditPatientAdditionalInfo;

const EditPatientAdditionalInfoButton = ({
  pending,
  reset,
}: {
  pending: boolean;
