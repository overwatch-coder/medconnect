...
const DiagnosticConversation = ({
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await mutateAsync({
      ...
      question: message,
      chatId: newChat ? undefined : selectedChat?.id,
    });
  };
  ...
  return (
    <section className="col-span-1 md:col-span-2 bg-white w-full relative flex flex-col rounded-t-xl shadow-md h-full">
      ...
      {/* Messages */}
      <ScrollArea className="flex h-[500px] flex-col gap-3 bg-chat-bg bg-no-repeat bg-cover bg-center w-full px-5 py-3 overflow-y-scroll scrollbar-hide pb-10">
        ...
        {newChat ? (
          !selectedPatient && (
            <div className="flex flex-col items-center justify-center gap-5 w-full h-full mx-auto min-h-full pt-10">
              <p className="text-sm flex flex-col justify-center items-center gap-2">
          ...
          <div className="flex flex-col gap-5 w-full min-h-full h-full pt-10 pb-5">
            ...
            {pending && (
              <div className="flex flex-col gap-2 w-1/2 p-5 text-sm rounded-lg shadow-md bg-primary-gray/10 self-start text-secondary-gray items-start">
                <PulseLoader size={5} loading={pending} color="#333333" />
              </div>
            )}
          </div>
        )}
