...
type AddHealthOfficialInfoProps = {
  open: boolean;
...
const AddHealthOfficialInfo = ({
  open,
  setOpen,
}: AddHealthOfficialInfoProps) => {
  const [user] = useAuth();
  ...
  const {
    ...
    defaultValues: {
      chpsCompoundId: user?.staff?.chpsCompoundId!,
    },
    mode: "all",
  });
  ...
  const {
    ...
    mutationFn: async (data) => createOrEditStaff({ data: data }),
    config: {
...
export default AddHealthOfficialInfo;
